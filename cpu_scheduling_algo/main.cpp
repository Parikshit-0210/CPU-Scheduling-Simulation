    ...
    for(int time =0;time<last_instant;time++){
        ...
        for(int i=0;i<v.size();i++){
            if(get<1>(v[i])==currentProcess){
                ...
                get<0>(v[i])++;//Increase Priority Level
                get<2>(v[i])++;//Increase Waiting Time
            }
        }
...
void execute_algorithm(char algorithm_id, int quantum,string operation)
{
    ...
    {
    case '1':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"FCFS  ";
        firstComeFirstServe();
        break;
    case '2':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"RR-"<<quantum<<"  ";
        roundRobin(quantum);
        break;
    case '3':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"SPN   ";
        shortestProcessNext();
        break;
    case '4':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"SRT   ";
        shortestRemainingTime();
        break;
    case '5':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"HRRN  ";
        highestResponseRatioNext();
        break;
    case '6':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"FB-1  ";
        feedbackQ1();
        break;
    case '7':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"FB-2i ";
        feedbackQ2i();
        break;
    case '8':
        if(operation==TRACE || operation == SHOW_BOTH)cout<<"Aging ";
        aging(quantum);
        break;
    default:
        break;
...
int main()
{
    int a = 0;
    do{
        cout << "Index for Algorithm:\n";
        for(int algo = 1;algo < 8;algo++){
            cout << ALGORITHMS[algo] << ":" << algo << ",";
        }
        cout << ALGORITHMS[8] << ":" << 8 << endl;
        parse();
        for (int idx = 0; idx < (int)algorithms.size(); idx++)
        {
            ...
            if (operation == TRACE){
                ...
                printStats(idx);
            else if (operation == SHOW_BOTH){
                printTimeline(idx);
                printStats(idx);
            }
            cout << "\n";
        }
        cout << "Want to do it again?:(yes:1,no:-1):";
        cin >> a;
        freeMemory();
        system("CLS");
    }while(a!=-1);
